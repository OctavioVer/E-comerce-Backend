...
export async function deleteProductInCart(req, res) {
  ...
  // CREO UNA NUEVA ARRAY SOLO CON LOS PRODUCTOS QUE NO COINCIDEN CON EL ID DEL PRODUCTO A ELIMINAR
  const newCartProducts = cart.productos.filter((producto) => {
    return producto.id != id_prod;
  });

  // ACTUALIZO EL CARRITO CON LA MATRIZ DE PRODUCTOS ACTUALIZADOS.
